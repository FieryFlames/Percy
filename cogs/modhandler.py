    ...
    async def on_member_ban(self, guild, user):
        async with self.sessionmaker() as session:
            async with session.begin():
                ...
                # delete that mf
                await role.delete(reason=f"{str(user)} (this custom role's primary user) was banned") # TODO: If you implement role sharing, make sure you do whatever you need to do to the other user's booster row
